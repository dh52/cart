...
var scores = [12, 2, 300, 53, 44, 67, 23, 52, 99, 144, 31];

colorList.pop();//this removes the end element amd returns the element
//assign the end element to a result variable
var endElement = colorList.pop();

//colorList.shift();//removes the first element from the array

//colorList.unshift("brown");//adds to the start of the list

colorList.splice(0, 2);//takes out one or many elements.  first argument is the starting position secon argument is how many to take out.
...
function buildThumbnails()
{
  ...
  output += '<div class="thumbs_block">'; //+= concantination output function
...
function onImageClick(evt)
{
  ...
  //the indexOf array method searches for a match in the array and if it finds the data, it sends bac the index position in the array
  ...
  purchasdImages.push(selectedImage);//push adds the element to the new array
  ...
  function update_shopping_cart()
{
  ...
  //create and output variable so we can build it up over the following lines
  var output = "";
  
  //create a for loop
  for(var i=0; i <= purchasdImages.length -1; i++ )
  //use a -1 after imageList.length so the condition is always one less (to match the index number)
    
  {
    output += '<img src="' + purchasdImages[i] + '" width="70" height="50" />';
  }
  ...
  //console.log(evt.target);//brings back just theelement that gets clicked
  
  //evt.target.setAttribute("src", "images/drawing/thumbs/artwork_15.jpg");
  
  //evt.target.setAttribute("style", "display; none; ");//error
